const StartFunc = e => {
    if (IscontinuousListening === false) {
        LocalFuncforBreak(e);
    } else {
        LocalFuncforContinuous(e);
    };
};

const LocalFuncforBreak = e => {
    if (activeSession !== session || !activeSection) return;
    const r = e.results[e.results.length - 1];
    if (r.isFinal) addNote(r[0].transcript.trim());
};

const LocalFuncforContinuous = e => {
    let transcript = "";
    for (let i = event.resultIndex; i < event.results.length; i++) {
        transcript += event.results[i][0].transcript;
    };
    const output = document.getElementById("showAllContent");
